  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="gap-2 gradient-primary text-primary-foreground"><Plus className="h-4 w-4" /> Create New Group</Button>
      </DialogTrigger>
      <DialogContent className="glass-card border-border/50 max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <Button onClick={handleCreate} disabled={createGroup.isPending} className="w-full gradient-primary text-primary-foreground font-semibold">
...
function JoinGroupDialog() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2"><UserPlus className="h-4 w-4" /> Join Existing</Button>
      </DialogTrigger>
      <DialogContent className="glass-card border-border/50 max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground text-center">Ask a friend to share their group's invite code</p>
          <Button onClick={handleJoin} disabled={joinGroup.isPending} className="w-full gradient-primary text-primary-foreground font-semibold">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card-elevated p-4 space-y-3">
      <div className="flex items-center gap-3">
        <div className="h-12 w-12 rounded-xl gradient-primary flex items-center justify-center flex-shrink-0"><Users className="h-6 w-6 text-primary-foreground" /></div>
        <div className="flex-1 min-w-0">
          <p className="font-display font-semibold truncate">{group.name}</p>
          {group.description && <p className="text-xs text-muted-foreground truncate">{group.description}</p>}
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground/60 mt-0.5">{group.myRole === "admin" ? "Admin" : "Member"}</p>
        </div>
      </div>
      <div className="flex items-center gap-2">
        <button onClick={() => setShowCode(!showCode)} className="flex-1 text-left text-xs text-muted-foreground hover:text-foreground transition-colors">
        ...
        <button onClick={copyInviteCode} className="p-1.5 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors"><Copy className="h-3.5 w-3.5 text-muted-foreground" /></button>
        <button onClick={shareInvite} className="p-1.5 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors"><Share2 className="h-3.5 w-3.5 text-muted-foreground" /></button>
        <button onClick={() => onLeave(group.id)} className="p-1.5 rounded-lg bg-destructive/10 hover:bg-destructive/20 transition-colors"><LogOut className="h-3.5 w-3.5 text-destructive" /></button>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-20 -left-40 h-60 w-60 rounded-full bg-accent/8 blur-[80px]" />
      </div>
      <div className="relative mx-auto max-w-md space-y-5 p-4">
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="pt-3 font-display text-2xl font-bold">Groups</motion.h1>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex gap-3">
          <CreateGroupDialog />
          <JoinGroupDialog />
        </motion.div>
        {isLoading ? (
          <div className="glass-card flex items-center justify-center py-12"><p className="text-sm text-muted-foreground">Loading groups...</p></div>
        ) : groups.length === 0 ? (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card-elevated flex flex-col items-center gap-3 py-10 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-secondary/80"><Users className="h-6 w-6 text-muted-foreground/60" /></div>
            <div><p className="font-display font-semibold">No groups yet</p><p className="mt-1 text-sm text-muted-foreground">Create a group or join one with an invite code</p></div>
          </motion.div>
        ) : (
